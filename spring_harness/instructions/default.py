...
def register_default_instructions(
    ...
    async def code_mode_rules(ctx: RunContext[CodingAgentDeps]) -> str:
        return (
            ...
            "`/scratch`. `/work` and `/scratch` are sandbox-only virtual paths — use them "
            "only inside `run_code` code, never as path arguments to top-level tools; "
            "top-level tools take workspace-relative paths like `src/main.py`. "
            "Environment variables and the clock are unavailable. State persists "
            "between `run_code` calls (REPL-style). "
            "Never echo the code in your reply text — the `run_code` call with its `code` "
            "argument is already visible to the user, so repeating the snippet in prose "
            "doubles output tokens. Write code only in the `code` argument and describe "

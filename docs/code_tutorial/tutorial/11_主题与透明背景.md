...
跑起来之后，界面和第 10 步**看起来几乎一样**：欢迎框、聊天、下拉框、弹窗全部照旧。这一步的改动都在「看不见的地方」——所有颜色不再散落在各个组件的 CSS 里写死，而是收进一个 `Theme` 对象统一管理；更关键的是，背景和前景文字都不再写死：整个 App 的背景从「固定深色」变成「跟随终端默认背景」（`ansi_default`，如果你的终端支持半透明，比如 Windows Terminal 开亚克力效果，窗口背后的桌面能透出来），前景也从写死的浅灰 `#c8cdd5` 改成 `ansi_default`，完全跟随终端配色。
...
- **ansi_default**：一个特殊的颜色名，意思是「终端自己的默认颜色」。用它做背景时，Textual 渲染输出的不是某个 RGB 色值，而是 ANSI 转义码 SGR `49`——等于告诉终端「这块背景你自己画」，终端是半透明的就能透出背景；用它做前景（文字）时输出 SGR `39`，文字颜色完全跟随终端配色方案。本步把 `foreground` 也从写死的浅灰 `#c8cdd5` 改成 `ansi_default`，前景、背景一起交给终端。
